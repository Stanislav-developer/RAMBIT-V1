...
#include <DNSServer.h> // ⚠️ НОВЕ: Бібліотека для автоматичного відкриття сторінки

HardwareSerial crsfSerial(2);
AlfredoCRSF crsf;
WebServer server(80);
Preferences preferences;
DNSServer dnsServer; // ⚠️ НОВЕ: Об'єкт DNS сервера
...
void runConfigMode() {
  ...
  // ⚠️ НОВЕ: Запускаємо DNS сервер для Captive Portal (порт 53)
  dnsServer.start(53, "*", WiFi.softAPIP());

  crsfSerial.begin(420000, SERIAL_8N1, RX_PIN, TX_PIN);
  crsf.begin(crsfSerial);

  // ⚠️ НОВЕ: Додано charset=utf-8 до всіх відповідей
  server.on("/", HTTP_GET, []() { 
  ...
  // ⚠️ НОВЕ: Captive Portal (перехоплення всіх невідомих запитів)
  // Якщо телефон намагається відкрити "gstatic.com" чи щось подібне для перевірки інтернету,
  // ми перенаправляємо його на нашу головну сторінку.
  server.onNotFound([]() {
  ...
  while (true) {
    dnsServer.processNextRequest(); // ⚠️ НОВЕ: Обслуговуємо DNS-запити для порталу
    server.handleClient();
    crsf.update();
    delay(2);
  }
